...
const RatingControllers = {
  createRating: asyncHandler(async (req, res) => {
    ...
    return res
      .status(200)
      .json({ message: "Rating successfully saved", rating });
  }),
  ...
  getRatingsForPost : async (req, res) => {
    ...
    try {
      const ratings = await db.Rating.findAll({ where: { idPost }, include: User });
      if (ratings.length === 0) {
        return res.status(404).json({ message: "No ratings found for this post" });
      }
