...
import usePrototypes from "@/zustand/usePrototypes";
...
const CardPrototypes = ({ setLayout, limit,isPersonal }) => {
  const { me } = useMeStore();
  ...
  useEffect(() => {
    ...
    //     idUser: me.id,
    //     // fields:
    //     //   "address,ward,images,createdAt,district,idPost,price,priceUnits,province,size,title",
    //     limit: limit,
    //     ...params,
    //   });
    ...
    const fetchPrototypes = async (params) => {
      const response = await apiGetPrototypes({
        idUser: isPersonal ? me.id : undefined, // Nếu là cá nhân thì lọc theo idUser
        limit,
        ...params,
      });
      ...
      if (response.data.success) {
        ...
        // Lọc bài đăng nếu không phải cá nhân (ẩn "Nháp" và "Chờ duyệt")
        // if (!isPersonal) {
        //   filteredProperties = filteredProperties.filter(
        //     (item) => item.status !== "Nháp" && item.status !== "Chờ duyệt"
        //   );
        // }
  ...
  const handleRemove = async (pid) => {
    const response = await apiDeletePostId(pid);
  };
  return (
              ...
              <div className="grid grid-cols-10 gap-4">
                <div className="col-span-4 relative">
                  <img
                    ...
                    className="object-cover w-full h-full"
                  />
                  {/* Badge chỉ hiển thị nếu isPersonal là true */}
                  {isPersonal && (
                    <>
                      <Badge
                        className={`absolute top-0 left-0 ${getStatusColor(
...
// const filteredItem = response.data.filter(item => item.status !== "Nháp" && item.status !== "Chờ duyệt");
