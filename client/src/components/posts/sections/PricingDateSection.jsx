...
const PricingDateSection = ({ form, handlePricingChange }) => {
  ...
  const handleOptionSelect = (option) => {
    ...
    if (me.balance >= option.pricePerDay * option.days) {
      ...
      form.setValue("expiredDate", new Date(Date.now() + option.days * 24 * 60 * 60 * 1000)); // Cập nhật ngày hết hạn
  ...
  return (
    <div className="w-full p-1 border-2 border-red-400">
      <h3 className="text-base font-medium mb-4">Đăng dài ngày hơn, tiết kiệm hơn!</h3>
        ...
        {pricingOptionsOfPost.map((option) => (
          <div key={option.days}>
            ...
            >
              ...
              <div className="text-sm">{option.pricePerDay.toLocaleString()} đ/ngày</div>
            </div>
          </div>
        ))}
      </div>
